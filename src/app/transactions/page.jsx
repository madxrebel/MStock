...
const TransactionsPage = () => {
  ...
  const uid = searchParams.get("uid"); // Get uid from the URL parameter

  useEffect(() => {
    if (!uid) return; // Ensure uid is available before fetching data

    const fetchTransactions = async () => {
      try {
        // Query transactions where createdBy is equal to the uid
        const transactionsQuery = query(collection(db, "transactions"), where("createdBy", "==", uid));
